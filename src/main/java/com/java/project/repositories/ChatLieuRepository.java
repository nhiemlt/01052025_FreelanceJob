...
public interface ChatLieuRepository extends JpaRepository<ChatLieu, Integer> {
    @Query("SELECT c FROM ChatLieu c WHERE c.tenChatLieu LIKE %:search% OR c.id = :search")
    Page<ChatLieu> findByIdOrTenChatLieu(@Param("search") String search, Pageable pageable);
